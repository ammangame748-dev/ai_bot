...
async function askAI(prompt, context, history) {
  ...
  const r = await fetch('https://api.cerebras.ai/v1/chat/completions', { method:'POST', headers:{'Content-Type':'application/json', Authorization:`Bearer ${process.env.CEREBRAS_API_KEY}`}, body:JSON.stringify({ model, temperature:.35, max_tokens:1800, messages:[{role:'system',content:'أنت مساعد Discord عربي ذكي ومحدث. تذكّر سياق الحوار السابق لهذا المستخدم واستخدمه عندما يكون مفيدًا. استخدم نتائج البحث المرفقة داخليًا عند الحاجة، أجب بشكل واضح ومفيد، لا تخترع معلومات، ولا تعرض روابط أو مصادر أو عناوين URL في الإجابة.'},{role:'user',content:`سجل الحوار السابق:\n${conversation || 'لا يوجد حوار سابق.'}\n\nالسؤال الحالي:\n${prompt}\n\nنتائج الويب الداخلية:\n${context || 'لم تظهر نتائج إضافية.'}`}] }) });
