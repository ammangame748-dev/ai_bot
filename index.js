...
async function askAI(prompt, context, history) {
  if (!process.env.CEREBRAS_API_KEY) return 'البوت متصل، لكن الدردشة الذكية تحتاج إضافة CEREBRAS_API_KEY في Render.';
  const model = process.env.CEREBRAS_MODEL || 'gpt-oss-120b';
  const conversation = (history || []).slice(-10).map(item => `${item.role === 'user' ? 'المستخدم' : 'المساعد'}: ${item.content}`).join('\n');
  const r = await fetch('https://api.cerebras.ai/v1/chat/completions', { method:'POST', headers:{'Content-Type':'application/json', Authorization:`Bearer ${process.env.CEREBRAS_API_KEY}`}, body:JSON.stringify({ model, temperature:.35, max_completion_tokens:1800, reasoning_effort:'low', messages:[{role:'system',content:'أنت مساعد Discord عربي ذكي ومحدث. تذكّر سياق الحوار السابق لهذا المستخدم واستخدمه عندما يكون مفيدًا. استخدم نتائج البحث المرفقة داخليًا عند الحاجة، أجب بشكل واضح ومفيد، لا تخترع معلومات، ولا تعرض روابط أو مصادر أو عناوين URL في الإجابة.'},{role:'user',content:`سجل الحوار السابق:\n${conversation || 'لا يوجد حوار سابق.'}\n\nالسؤال الحالي:\n${prompt}\n\nنتائج الويب الداخلية:\n${context || 'لم تظهر نتائج إضافية.'}`}] }) });
  const data = await r.json();
  if (!r.ok) throw new Error(data.error?.message || `Cerebras API error ${r.status}`);
...
client.on('messageCreate', async message => {
  ...
  try {
    ...
    if (e.message.includes('429')) return message.reply('وصلنا حد Cerebras المؤقت. سيعود البوت تلقائيًا بعد تجدد الحد.');
