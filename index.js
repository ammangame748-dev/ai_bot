...
app.get('/', requireLogin, (req, res) => {
  ...
  const selected = settings[manageable[0]?.id] || '';
  const guildOptions = manageable.length ? manageable.map(g => `<option value="${esc(g.id)}" ${selected === g.id ? 'selected' : ''}>${esc(g.name)}</option>`).join('') : '<option>لا توجد سيرفرات قابلة للإدارة</option>';
  const body = `<nav class="nav"><div class="brand">Nebula <span>AI</span></div><div class="user"><div class="avatar">${esc((req.session.user.username || 'U')[0].toUpperCase())}</div><span>${esc(req.session.user.username)}</span><a class="logout" href="/logout">خروج</a></div></nav><main class="wrap"><section class="hero"><div class="eyebrow">DISCORD AI ASSISTANT</div><h1>مرحبًا بك، ${esc(req.session.user.global_name || req.session.user.username)}</h1><p>تحكم بمكان الدردشة، ودع البوت يجيب بذكاء مع معلومات من الويب عند الحاجة.</p></section><div class="notice"><span class="status"><i class="dot"></i> ${client.isReady() ? 'البوت متصل ويعمل' : 'البوت يتصل الآن'}</span> — لا تضع مفاتيحك السرية داخل الكود، استخدم Environment Variables في Render.</div><section class="grid"><article class="card"><h2>تحديد روم الدردشة</h2><p>اختر السيرفر ثم الروم الذي يجب أن يرد فيه البوت. يجب أن يكون البوت موجودًا في السيرفر.</p><form method="post" action="/settings"><select class="select" name="guildId" required>${guildOptions}</select><select class="select" name="channelId" id="channelId" required><option value="">اختر السيرفر أولًا</option></select><button class="button">حفظ الإعدادات</button></form></article><article class="card"><h2>حالة النظام</h2><p>يستمع البوت للرسائل في الروم المحدد فقط، ويمنع الردود المتكررة على نفسه.</p><div class="hint">البحث بالويب: ${process.env.WEB_SEARCH_ENABLED === 'false' ? 'متوقف' : 'مفعّل'}<br>نموذج الذكاء: ${esc(process.env.AI_MODEL || 'يُحدد من Render')}<br>الاستضافة: Render</div></article></section><div class="footer">Nebula AI · لوحة تحكم Discord</div></main><script>const guilds=${JSON.stringify(manageable.map(g=>({id:g.id,name:g.name})))};const sel=document.querySelector('[name=guildId]'),ch=document.querySelector('#channelId');async function load(){if(!sel.value)return;ch.innerHTML='<option>جار تحميل الرومات...</option>';const r=await fetch('/api/channels/'+sel.value);const d=await r.json();ch.innerHTML=d.map(x=>'<option value="'+x.id+'">'+x.name+'</option>').join('')||'<option>لا يوجد روم نصي</option>'}sel.addEventListener('change',load);load();</script>`;
